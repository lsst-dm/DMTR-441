...
% using template at /opt/hostedtoolcache/Python/3.11.16/x64/lib/python3.11/site-packages/docsteady/templates/tpr.latex.jinja2.
...
\begin{minipage}[]{9cm}
\smallskip
Test executed at the USDF using pipelines version `w\_2026\_02`. The
...
\begin{minipage}[]{9cm}
\smallskip
Executed at the USDF using pipelines version `w\_2026\_13`. The script
...
% Note Steps "Not Executed" and with No Result are not shown in this report if the flag is passed
Detailed steps results LVV-R293-LVV-E3756-1319223017:\\
  {

\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3756-1 & Step Execution Status: \textbf{ Not Executed } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
We have assessed performance of the observatory and Camera for
commissioning see SITCOM-TN-149 and DP1 (RTN-95). Hence we say this is
ok.~~
...
% Note Steps "Not Executed" and with No Result are not shown in this report if the flag is passed
Detailed steps results LVV-R293-LVV-E3762-1319223024:\\
  {

\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3762-1 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Open a HiPS image in the "Rubin HiPS Search" tab of the RSP Portal
aspect. Use the "overlays" tool to select a MOC layer and display it.
Confirm that the MOC displays as expected.~
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
In the RSP at data.lsst.cloud, navigate to the Portal, then select
"Rubin HiPS Search". Search at position "95.0 -25.0 Equ J2000", and
select one of the various color HiPS maps. Once it is loaded, click on
the "overlays" tool and select "MOC - LSSTComCam: DP1 gri". Zoom to the
appropriate scale, and observe that the MOC appears as expected. The
following screenshot illustrates that the MOC (in red) is displayed:\\
\textbf{Image Download Error}
...
The y-axis of this plot is the mean duration of queries in seconds,
which is roughly flat at \textasciitilde400 seconds (roughly 6-7
minutes) after some initial bottlenecks. The requirement is thus met, as
this is much less than the required 1 hour.
...
{\# High-volume complex query involving full-sky spatial and temporal
correlations and use of some internal functions } {\# Mag 17 cut:
Results in a 1.1 GB dataframe, greater than the limit for "small
queries" of 0.5GB} query {=} {"""SELECT coord\_ra,coord\_dec,
scisql\_nanojanskyToAbMag(r\_scienceFluxMean) as rScienceMag} { FROM
dp1.DiaObject AS diao} { JOIN dp1.ForcedSourceOnDiaObject as fsodo ON
fsodo.diaObjectId = diao.diaObjectId} { JOIN dp1.Visit as vis ON
vis.visit = fsodo.visit } { where
scisql\_nanojanskyToAbMag(r\_scienceFluxMean) \textless{} 23} {"""}
Query execution time is monitored via a Chronograph dashboard, which
yields the following query execution times from the test: \textbf{Image
Download Error} The y-axis of this plot is the mean duration of queries
in seconds, which is roughly flat at \textasciitilde6800 seconds (less
than 2 hours). The requirement is thus met, as this is much less than
...
% Note Steps "Not Executed" and with No Result are not shown in this report if the flag is passed
Detailed steps results LVV-R293-LVV-E3804-1319223071:\\
  {

\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3804-1 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
The following are screenshots from various grafana dashboards used to
monitor prompt processing. The first screenshot demonstrates that the
processing is running on hundreds of workers:\textbf{Image Download
Error}The following screenshot shows the processing time for many tasks
executed by the prompt processing, demonstrating that the system is
capable of processing images in nearly real-time.\\
\textbf{Image Download Error}Finally, we show an alert production and
prompt processing status dashboard that is used to monitor the AP and
Prompt services themselves.\\
\textbf{Image Download Error}\\
Links to the various dashboards:\\
prompt
overview:~\url{https://grafana.slac.stanford.edu/d/feifuf82d6tj4f/prompt-processing-production-overview}\\
prompt processing
statistics:~\url{https://grafana.slac.stanford.edu/d/c0a9c6d3-4ea8-452d-bb91-962304c1b0d2/prompt-processing-production-statistics}\\
prompt timing:~\url{https://grafana.slac.stanford.edu/d/eeimq740rdgjkf/}
...
% Note Steps "Not Executed" and with No Result are not shown in this report if the flag is passed
Detailed steps results LVV-R293-LVV-E3816-1319223083:\\
  {

\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3816-1 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Identify a dataset that has been processed through coaddition and object
table creation.
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
We select "dp2\_prep" data processing that used the release candidate
`v30\_0\_0\_rc2` version of the pipelines. Its repo/collection were
initialized via the following:\\
\strut \\
butler = Butler(\textquotesingle dp2\_prep\textquotesingle,
collections="LSSTCam/runs/DRP/20250515-20251214/v30\_0\_0\_rc2/DM-53697")
...
\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3816-2 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Examine an Object catalog and confirm that it has measurements of
deblended objects from the deep coadd images.
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
In the attached notebook, we examined the Object catalog at a
randomly-selected position, and confirmed that it contains measurements
of deblended objects. Object coordinates were overlaid on a
corresponding `deep\_coadd` image to confirm that overlapping sources
have been deblended.\\
\strut \\
At present, the Object catalog does not contain information from
difference imaging or single-epoch images. The Object table will
continue to evolve in future data releases and processing campaigns.
...
% Note Steps "Not Executed" and with No Result are not shown in this report if the flag is passed
Detailed steps results LVV-R293-LVV-E3817-1319223084:\\
  {

\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3817-1 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Perform the steps of Alert Production (including, but not necessarily
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
Performed as part of the prompt processing commissioning~
...
\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3817-2 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Verify that the expected data products have been produced, and that
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
All data products stated in the requirement are present for inclusion in
the alert packets, except for mask information in the cutouts. This is
awaiting a bug fix implementation in ~astropy v8. We\textquotesingle re
on v7.2 right now \url{https://github.com/astropy/astropy/pull/18862}
...
\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3817-3 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
The alert packets successfully sent to brokers, containing all
information except the ~masks on the cutouts.~
...
% Note Steps "Not Executed" and with No Result are not shown in this report if the flag is passed
Detailed steps results LVV-R293-LVV-E3844-1319223116:\\
  {

\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3844-1 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
We have been commissioning the system with LSSTCam for nearly a year,
and have always been able to keep up with the daily processing with no
major backlogs. We thus consider that demonstration that the pipeline
throughput is sufficient for sustained observing.
...
% Note Steps "Not Executed" and with No Result are not shown in this report if the flag is passed
Detailed steps results LVV-R293-LVV-E3848-1319223120:\\
  {

\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3848-1 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Identify the path to the data repository, which we will refer to as
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
The following code was executed to point to a recently processed DP2
prep collection:\\
\strut \\
from lsst.daf.butler import Butler\\
from lsst.pipe.base.pipeline\_graph.visualization import show\\
\strut \\
\# Initialize the Butler pointing to recently-processed data\\
repo = "dp2\_prep"\\
coll =
\textquotesingle LSSTCam/runs/DRP/DP2-pilot/v30\_0\_4\_rc1/DM-54210/stage3\textquotesingle{}\\
...
\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3848-2 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Retrieve a deep coadd image from the Butler.
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
All steps were executed as part of a script from the command line at the
USDF. The script was executed as follows, which includes piping the
output to a log file:\\
\strut \\
python test\_LVV-T64.py 2\textgreater\&1 \textbar{} tee
test\_LVV-T64.log\\
\strut \\
The log contains the following output notifying which `deep\_coadd`
image was randomly selected:\\
\strut \\
Selected DatasetRef: deep\_coadd@\{band:
\textquotesingle i\textquotesingle, skymap:
\textquotesingle lsst\_cells\_v2\textquotesingle, tract: 9813, patch:
70\} {[}sc=ExposureF{]}
(run=LSSTCam/runs/DRP/DP2-pilot/v30\_0\_4\_rc1/DM-54210/stage3/20260220T185213Z
id=019c7c6b-4e13-7070-9c97-09abadbac187)\\
\strut \\
...
\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3848-3 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Retrieve the list of coadd inputs that contributed to the coadd image,
and examine them.
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
The output log shows the results of executing "coadd\_inputs =
coadd.info.getCoaddInputs()" and then extracting the visit and ccd
lists:\\
\strut \\
Coadd input visits\\
\strut ~~id bbox\_min\_x bbox\_min\_y bbox\_max\_x bbox\_max\_y ~
~~visit ~ ~\\
\strut ~ ~ ~ ~pix~ ~ ~ ~~pix~ ~ ~ ~~pix~ ~ ~ ~~pix ~ ~ ~ ~ ~ ~ ~ ~ ~\\
-\/-\/- -\/-\/-\/-\/-\/-\/-\/-\/-\/- -\/-\/-\/-\/-\/-\/-\/-\/-\/-
-\/-\/-\/-\/-\/-\/-\/-\/-\/- -\/-\/-\/-\/-\/-\/-\/-\/-\/-
-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500122\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500123\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500124\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500125\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500126\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500127\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500128\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500129\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500130\\
...~ ~ ~ ~~...~ ~ ~ ~~...~ ~ ~ ~~...~ ~ ~ ~~... ~ ~ ~ ~ ~~...\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025121600090\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025121600091\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025121900235\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025121900236\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025123100242\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025123100243\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025123100246\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025123100247\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100266\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100267\\
Length = 123 rows\\
-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\\
\strut \\
Coadd input CCDs\\
...
-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/- -\/-\/-
-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500122 139
0.00044828246\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500122 142
0.00044961445\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500122 143
0.00043541516\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500123
139~~0.0006545108\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500123
...
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500123
142~~0.0006375272\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500123 143
0.00061971653\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500124
139~~0.0007354588\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025051500124 140
0.00072857214\\
...~ ~ ~ ~~...~ ~ ~ ~~...~ ~ ~ ~~...~ ~ ~ ~~... ~ ~ ~ ~ ~~... ... ~ ~ ~
~ ~~...\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025123100247
...
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2025123100247
109~~0.0006132769\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100266 104
0.00046470456\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100266
107~~0.0004648852\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100266 111
0.00046319223\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100266 114
0.00046469842\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1
2026010100267~~62~~0.0004659005\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100267~~69
0.00046485366\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100267 101
0.00045227894\\
\strut ~~0~ ~ ~ ~ ~~0~ ~ ~ ~ ~~0 ~ ~ ~ ~~-1 ~ ~ ~ ~~-1 2026010100267 108
0.00046505127\\
Length = 436 rows\\
\strut \\
...
\begin{tabular}{p{4cm}p{12cm}}
\toprule
Step LVV-E3848-4 & Step Execution Status: \textbf{ Pass } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
Query and verify provenance of the configuration and software versions
that went into producing the coadd image.
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
The script continues to display the quantum graph for this run, as well
as list the configuration and package versions that were used. The
following shows a portion of the log output:\\
\strut \\
Configuration for assembleDeepCoadd task:\\
import lsst.drp.tasks.assemble\_coadd\\
assert type(config) is
lsst.drp.tasks.assemble\_coadd.CompareWarpAssembleCoaddConfig, f"config
is of type \{type(config).\_\_module\_\_\}.\{type(config).\_\_name\_\_\}
instead of
lsst.drp.tasks.assemble\_coadd.CompareWarpAssembleCoaddConfig"\\
\strut \\
...
\# Flag to enable/disable saving of log output for a task, enabled by
default.\\
config.saveLogOutput=True\\
\strut \\
\# Coadd name: typically one of deep or goodSeeing.\\
config.coaddName=\textquotesingle deep\textquotesingle{}\\
\strut \\
\# Flag to enable/disable saving of log output for a task, enabled by
default.\\
config.select.saveLogOutput=True\\
\strut \\
\# Maximum PSF FWHM (in arcseconds) to select. This can be used in
conjunction with the per-visit maxPsfFwhm in BestSeeingSelectVisitsTask
to ensure that no outlier CCD has a PSF FWHM larger than this value.\\
config.select.maxPsfFwhm=1.8\\
\strut \\
PACKAGES:\\
\strut \\
Packages used for run:\\
Packages(\{\\
\textquotesingle Bottleneck\textquotesingle:\textquotesingle1.6.0\textquotesingle,\\
\textquotesingle Brotli\textquotesingle:\textquotesingle1.1.0\textquotesingle,\\
\textquotesingle Deprecated\textquotesingle:\textquotesingle1.3.1\textquotesingle,\\
\textquotesingle Flask\textquotesingle:\textquotesingle3.1.3\textquotesingle,\\
\textquotesingle GaiaXPy\textquotesingle:\textquotesingle2.1.2\textquotesingle,\\
\textquotesingle GalSim\textquotesingle:\textquotesingle2.8.3\textquotesingle,\\
\textquotesingle GitPython\textquotesingle:\textquotesingle3.1.46\textquotesingle,\\
\textquotesingle Jinja2\textquotesingle:\textquotesingle3.1.6\textquotesingle,\\
\textquotesingle LSSTDESC.Coord\textquotesingle:\textquotesingle1.3.1\textquotesingle,\\
\strut \\
We have thus demonstrated that the pipelines are producing sufficient
provenance to recreate coadd images.
...
\hfill\break
\pandocbounded{\includegraphics[keepaspectratio]{jira_imgs//zephyr3.png}}
...
u band: PF2 (outlier percentage): 0.00 \% g band: PF2 (outlier
percentage): 0.26 \% r band: PF2 (outlier percentage): 0.21 \% i band:
PF2 (outlier percentage): 0.19 \% z band: PF2 (outlier percentage): 0.00
\% y band: PF2 (outlier percentage): 0.00 \%
...
\end{tabular}
 Description \\
{\footnotesize
​Select a single visit+detector image, and re-run template creation
and ​difference imaging for that image. Specify~
